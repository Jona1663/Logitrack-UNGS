...
public class ReporteService {
        ...
        public ReporteSimpleDTO obtenerReporte(LocalDate fechaInicio, LocalDate fechaFin) {
                ...
                if (fechaInicio != null && fechaFin != null) {
                        ...
                        return ReporteSimpleDTO.builder()
                                        .totalViajes(totalViajes).totalKilos(totalKilos != null ? totalKilos : 0L)
                                        .totalIncidencias(totalIncidencias)
                                        .build();
                }

                // Histórico (Trae TODO el histórico)
        ...
        public ReporteEficienciaDTO obtenerMetricasATiempo(LocalDate fechaInicio, LocalDate fechaFin) {
                ...
                // Obtenemos cuántos llegaron a tiempo y sus kilos
                ...
                // Necesitamos el total de viajes completados para sacar el porcentaje real
        ...
        public List<ViajeCumplimientoDTO> calcularDesviosYCumplimiento(LocalDate fechaInicio, LocalDate fechaFin) {
                ...
                // Buscamos solo los envíos completados del repositorio
                ...
                // Iteramos cada envío para calcular la diferencia exacta contra el ETA
                for (Envio envio : enviosCompletados) {
                        ...
                        if (envio.getFechaEstimadaLlegada() != null && envio.getFechaLlegada() != null) {
                                minutosDiferencia = ChronoUnit.MINUTES.between(envio.getFechaEstimadaLlegada(),
                                                envio.getFechaLlegada());
                        }
                        ...
                        // Evaluamos de forma segura si el estado es nulo antes de transformarlo a
                        // String
                        String estadoFormateado = (envio.getEstadoActual() != null)
                                        ? envio.getEstadoActual().toString()
                                        : "DESCONOCIDO";

                        // Mapeamos los datos calculados al DTO individual
        ...
        public ReporteCumplimientoResponse obtenerReporteCumplimiento(LocalDate fechaInicio, LocalDate fechaFin) {
                // Obtenemos los viajes individuales con sus desvíos ya calculados 
                ...
                // Contamos cuántos llegaron a tiempo y cuántos con retraso
                ...
                // Calculamos los porcentajes de forma segura (evitando dividir por cero)
                ...
                // Armamos el DTO de métricas globales
                ...
                // Retornamos la respuesta consolidada final
        ...
        private String formatearDesvio(double horas) {
                ...
                if (dias > 0)
                        return dias + " d de retraso";
                return horasRestantes + " h de retraso";
        }

        // export en fformato que solicita el frontend
        @Transactional(readOnly = true)
        public void exportarViajesCumplimientoStreamCsv(LocalDate fechaInicio, LocalDate fechaFin,
                        java.io.Writer writer) throws java.io.IOException {
                LocalDateTime inicio = fechaInicio.atStartOfDay();
                LocalDateTime fin = fechaFin.atTime(23, 59, 59);
                java.time.format.DateTimeFormatter formateador = java.time.format.DateTimeFormatter
                ...
                try (java.util.stream.Stream<Envio> streamEnvios = envioRepository
                                .obtenerEnviosComoStreamParaExportacion(inicio, fin);
                                org.apache.commons.csv.CSVPrinter printer = new org.apache.commons.csv.CSVPrinter(
                                                writer, formato)) {

                        streamEnvios
                                        .filter(envio -> envio.getEstadoActual() != null
                                                        && envio.getEstadoActual().name().equals("ENTREGADO"))
                                        .forEach(envio -> {
                                                try {
                                                        double desvioHoras = 0;
                                                        if (envio.getFechaLlegada() != null
                                                                        && envio.getFechaEstimadaLlegada() != null) {
                                                                long minutosDiferencia = java.time.temporal.ChronoUnit.MINUTES
                                                                                .between(
                                                                                                envio.getFechaEstimadaLlegada(),
                                                                                                envio.getFechaLlegada());
                                                                if (minutosDiferencia > 0) {
                                                        ...
                                                        printer.printRecord(
                                                                        ...
                                                                        envio.getFechaEstimadaLlegada() != null ? envio
                                                                                        .getFechaEstimadaLlegada()
                                                                                        .format(formateador) : "",
                                                                        envio.getFechaLlegada() != null ? envio
                                                                                        .getFechaLlegada()
                                                                                        .format(formateador) : "",
        ...
        @Transactional(readOnly = true)
        public void exportarReporteOperativoCsv(LocalDate fechaInicio, LocalDate fechaFin, java.io.Writer writer)
                        throws java.io.IOException {
                // Métricas Globales
                ReporteSimpleDTO totales = obtenerReporte(fechaInicio, fechaFin);

                // Si no hay datos, lanzamos excepción para que el controlador devuelva un JSON ---
                if (totales == null || totales.getTotalViajes() == 0) {
                        throw new RuntimeException("EMPTY_STATE");
                }

                List<ReporteEstadoDTO> estados = (fechaInicio != null && fechaFin != null)
                                ? envioRepository.obtenerMetricasPorEstadoEntreFechas(fechaInicio.atStartOfDay(),
                ...
                // Cambiamos al formato DEFAULT (separado por comas ,)
                ...
                try (org.apache.commons.csv.CSVPrinter printer = new org.apache.commons.csv.CSVPrinter(writer,
                        ...
                        printer.printRecord("Estado: En Punto de Recolección",
                                        mapaEstados.getOrDefault("EN_PUNTO_RECOLECCION", 0L));
                        ...
                        if (granos != null && !granos.isEmpty()) {
                                for (ReporteGranoDTO grano : granos) {
                                        printer.printRecord("Grano: " + grano.getTipoGrano(),
                                                        grano.getCantidadEnvios());
                                }
                        }
        ...
        @Transactional(readOnly = true)
        public java.io.ByteArrayInputStream generarExcelOperativo(LocalDate fechaInicio, LocalDate fechaFin)
                        throws java.io.IOException {
                ReporteSimpleDTO totales = obtenerReporte(fechaInicio, fechaFin);
                if (totales == null || totales.getTotalViajes() == 0) {
                        throw new RuntimeException("EMPTY_STATE");
                ...
                try (org.apache.poi.ss.usermodel.Workbook workbook = new org.apache.poi.xssf.usermodel.XSSFWorkbook();
                        ...
                        // Usamos un array de 1 posición para poder incrementarlo dentro del Lambda
                        int[] rowNum = { 1 };
                        ...
                        java.util.function.BiConsumer<String, Object> addRow = (metrica, valor) -> {
                                ...
                                if (valor instanceof Long)
                                        row.createCell(1).setCellValue((Long) valor);
                                else if (valor instanceof Integer)
                                        row.createCell(1).setCellValue((Integer) valor);
                                else if (valor instanceof Double)
                                        row.createCell(1).setCellValue((Double) valor);
                                else
                        ...
                        addRow.accept("Estado: En Punto de Recolección",
                                        mapaEstados.getOrDefault("EN_PUNTO_RECOLECCION", 0L));
        ...
        @Transactional(readOnly = true)
        public java.io.ByteArrayInputStream generarExcelCumplimiento(LocalDate fechaInicio, LocalDate fechaFin)
                        throws java.io.IOException {
                List<ViajeCumplimientoDTO> viajes = calcularDesviosYCumplimiento(fechaInicio, fechaFin);
                ...
                try (org.apache.poi.ss.usermodel.Workbook workbook = new org.apache.poi.xssf.usermodel.XSSFWorkbook();
                        ...
                        for (ViajeCumplimientoDTO viaje : viajes) {
                                ...
                                row.createCell(2)
                                                .setCellValue(viaje.getFechaEstimadaLlegada() != null
                                                                ? viaje.getFechaEstimadaLlegada().format(fmt)
                                                                : "");
                                row.createCell(3)
                                                .setCellValue(viaje.getFechaEntregaReal() != null
                                                                ? viaje.getFechaEntregaReal().format(fmt)
                                                                : "");
        ...
        public List<ReporteEnvioDetalleDTO> obtenerDetalleEnvios(LocalDate fechaInicio, LocalDate fechaFin) {
                ...
                // Usamos el método de Stream que funciona bien
        ...
        @Transactional(readOnly = true)
        public void exportarDetalleEnviosCsv(LocalDate fechaInicio, LocalDate fechaFin, java.io.Writer writer)
                        throws java.io.IOException {
                ...
                org.apache.commons.csv.CSVFormat formato = org.apache.commons.csv.CSVFormat.DEFAULT.builder()
                                .setHeader("ID Envío", "Estado", "Kilos Transportados", "Tipo de Grano",
                                                "Fecha Estimada Llegada", "Fecha Llegada Real")
                                .build();
                ...
                try (org.apache.commons.csv.CSVPrinter printer = new org.apache.commons.csv.CSVPrinter(writer,
                        ...
                        for (ReporteEnvioDetalleDTO dto : detalles) {
                                printer.printRecord(
                                                ...
                                                dto.getTipoGrano(),
                                                dto.getFechaEstimadaLlegada() != null
                                                                ? dto.getFechaEstimadaLlegada().format(dtf)
                                                                : "-",
        ...
        @Transactional(readOnly = true)
        public java.io.ByteArrayInputStream generarExcelDetalleEnvios(LocalDate fechaInicio, LocalDate fechaFin)
                        throws java.io.IOException {
                List<ReporteEnvioDetalleDTO> detalles = obtenerDetalleEnvios(fechaInicio, fechaFin);
                try (org.apache.poi.ss.usermodel.Workbook workbook = new org.apache.poi.xssf.usermodel.XSSFWorkbook();
                                java.io.ByteArrayOutputStream out = new java.io.ByteArrayOutputStream()) {
                        ...
                        font.setBold(true);
                        headerStyle.setFont(font);
                        String[] columnas = { "ID Envío", "Estado", "Kilos Transportados", "Tipo de Grano",
                                        "Fecha Estimada Llegada", "Fecha Llegada Real" };
                        org.apache.poi.ss.usermodel.Row headerRow = sheet.createRow(0);
                        for (int i = 0; i < columnas.length; i++) {
                                org.apache.poi.ss.usermodel.Cell cell = headerRow.createCell(i);
                        ...
                        for (ReporteEnvioDetalleDTO dto : detalles) {
                                ...
                                row.createCell(2).setCellValue(
                                                dto.getKilosTransportados() != null ? dto.getKilosTransportados() : 0);
                                row.createCell(3).setCellValue(dto.getTipoGrano());
                                row.createCell(4)
                                                .setCellValue(dto.getFechaEstimadaLlegada() != null
                                                                ? dto.getFechaEstimadaLlegada().format(dtf)
                                                                : "-");
                                row.createCell(5)
                                                .setCellValue(dto.getFechaLlegada() != null
                                                                ? dto.getFechaLlegada().format(dtf)
                                                                : "-");
                        }
                        for (int i = 0; i < columnas.length; i++) {
                                sheet.autoSizeColumn(i);
        ...
        public CumplimientoMetricasDTO obtenerMetricasCumplimiento(LocalDateTime fechaInicio, LocalDateTime fechaFin) {
                ...
                double pctRetraso = (total == 0) ? 0.0
                                : Math.round(((double) conRetraso / total * 100) * 100.0) / 100.0;
        ...
        public byte[] exportarCumplimientoExcel(LocalDateTime fechaInicio, LocalDateTime fechaFin) {
                ...
                try (Workbook workbook = new XSSFWorkbook();
                        ...
                        // Crear la fila de encabezados (Fila 0)
                        Row headerRow = sheet.createRow(0);
                        String[] columnas = { "Total Entregados", "Entregados a Tiempo", "Con Retraso", "% a Tiempo",
                                        "% con Retraso" };
