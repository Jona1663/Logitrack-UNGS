...
public class ChoferController {//Define la clase ChoferController, que es el controlador REST que manejará las solicitudes relacionadas con los choferes.
    ...
    @GetMapping("/envios")
    public ResponseEntity<List<Envio>> getMisEnvios(Authentication authentication) {//Define un método que responderá a solicitudes HTTP GET en la ruta "/envios" relativa al controlador (es decir, "/api/chofer/envios"). El método devuelve un ResponseEntity que envuelve una lista de objetos Envio. El Authentication es inyectado automáticamente por Spring Security y representa la identidad del usuario autenticado. Se usará para obtener el username del chofer y luego buscar los envíos asociados a ese chofer.                              
        ...
        return ResponseEntity.ok(misEnvios);
    }
}